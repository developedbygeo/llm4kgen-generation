...
export const createDbIndicesToolWithOntologyRAG = new DynamicTool({
    name: TOOL_NAMES.CREATE_DB_INDICES_WITH_ONTOLOGY,
    description: `Creates indices for the database based on the relationships identified between entities in the data by using the OWL ontology from the RAG.`,
    func: async (input: string) => {
        const processedInput = JSON.parse(input).map((result: string) =>
            processCsvLines(result)
        );
        ...
        const prompt = constructPromptFoDbIndicesWithOntologyRAG(
            processedInput,
            retrievedContext
        );
...
const constructPromptFoDbIndicesWithOntologyRAG = (
